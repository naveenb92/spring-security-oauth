...
public class JwtUserAuthenticationConverter implements UserAuthenticationConverter {
    ...
    public Authentication extractAuthentication(Map<String, ?> map) {
        if (map.containsKey(USERNAME)) {
            ...
            // Construct a principal by extracting custom info from token
            ApplicationUser principal = new ApplicationUser(map.get(USERNAME).toString(),"N/A",authorities);
            principal.setId(Long.parseLong(map.get("userId").toString()));
            principal.setTenantId(map.get("tenantId").toString());
